...
export function OtherProfile() {
  ...
  useEffect(() => {
    setIsLoading(true)
    fetchUserByID(user_id).then((data)=>{
      setOtherUser(data)
      setIsLoading(false)
    })
  }, [])
    ...
    <ScrollView>
      <BaseLayout>
        ...
        <Surface style={{width:"100%", alignItems:"center", justifyContent: "center", padding:10, borderRadius:10, marginBottom:20}}>
          <Text style={{width:"100%", fontSize:20, marginBottom:10}}>{otherUser.first_name} - {otherUser.xp} XP</Text>
        ...
        <Surface style={{width:"100%", alignItems:"center", justifyContent: "center", padding:10, borderRadius:10}}>
          <Text style={{width:"100%", fontSize:20, marginBottom:10}}>{otherUser.first_name}'s Most Recycled Item:</Text>
          <Image style={{
              width: 150,
              height: 150,
              marginBottom: 20,
